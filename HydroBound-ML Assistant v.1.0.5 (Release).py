...
class HydroBound_ML_App(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("HydroBound-ML Assistant v.1.0.5")
    ...
    def _setup_ui(self):
        ...
        # New Utility Button for loading intermediate results - style removed to match other buttons
    ...
    def load_existing_polygon(self):
        """
        Loads an existing GeoJSON polygon from the OUTPUT directory, bypassing the ML prediction step.
        Updates the internal vector_mask and displays it on the canvas for immediate export/editing.
        """
        ...
        try:
            ...
            # Odrzucenie geometrii, które nie są poligonami (np. punkty, linie)
            ...
            # Transform CRS to match the current canvas (raster) CRS
            ...
            # Wymuszenie resetu narzędzi pomiarowych i edycyjnych
            self.edit_mode = None
    ...
    def train_model(self):
        ...
        try:
            ...
            joblib.dump(
                {'X': X_manual_all, 'y': y_manual_all, 'coords': coords_manual_all, 'version': f'1.0.0_v{new_v}'},
                new_manual_path)
            ...
            if len(X_manual_all) > 5:
                X_m_tr, X_m_val, y_m_tr, y_m_val = train_test_split(X_manual_all, y_manual_all, test_size=0.2,
                                                                    random_state=42)
            else:
    ...
    def predict_mask(self):
        ...
        try:
            ...
            if model_exists:
                ...
                water_mask = cached_full_mask[aoi_window.row_off:aoi_window.row_off + aoi_window.height,
                             aoi_window.col_off:aoi_window.col_off + aoi_window.width]
                ...
                X_w = np.column_stack((
                    ...
                    scale_features[0][w_rows, w_cols], scale_features[1][w_rows, w_cols],
                    scale_features[2][w_rows, w_cols], scale_features[3][w_rows, w_cols],
                    scale_features[4][w_rows, w_cols], scale_features[5][w_rows, w_cols],
                    scale_features[6][w_rows, w_cols], scale_features[7][w_rows, w_cols],
                    scale_features[8][w_rows, w_cols], scale_features[9][w_rows, w_cols],
                    scale_features[10][w_rows, w_cols]
                ))
                X_l = np.column_stack((
                    ndwi[l_rows, l_cols], ndvi[l_rows, l_cols], ngrdi[l_rows, l_cols],
                    scale_features[0][l_rows, l_cols], scale_features[1][l_rows, l_cols],
                    scale_features[2][l_rows, l_cols], scale_features[3][l_rows, l_cols],
                    scale_features[4][l_rows, l_cols], scale_features[5][l_rows, l_cols],
                    scale_features[6][l_rows, l_cols], scale_features[7][l_rows, l_cols],
                    scale_features[8][l_rows, l_cols], scale_features[9][l_rows, l_cols],
                    scale_features[10][l_rows, l_cols]
                ))

                X_train_base = np.vstack((X_w, X_l))
                y_train_base = np.concatenate((np.ones(len(X_w)), np.zeros(len(X_l))))
                coords_base = np.vstack(
                    (np.column_stack((coords_w_x, coords_w_y)), np.column_stack((coords_l_x, coords_l_y))))

                joblib.dump({'X': X_train_base, 'y': y_train_base, 'coords': coords_base, 'version': '1.0.0'},
                            os.path.join(self.model_dir, "HydroBound-ML_otsu_samples_v1.0.0.joblib"))
    ...
    def export_geojson(self):
        ...
        Includes automated CRS reprojection with precision control.
        """
        if self.vector_mask is None or self.vector_mask.empty:
        ...
        try:
            ...
            if self.check_export_grid.isChecked():
                ...
                # Geometria 2D jest zachowana zgodnie z wymaganiami RiPROCESS
