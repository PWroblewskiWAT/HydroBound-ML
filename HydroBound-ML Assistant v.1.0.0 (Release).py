...
class BandSelectionDialog(QDialog):
    """
    Dialog window for mapping physical raster bands to spectral channels (Red, Green, Blue, NIR).
    Prevents spatial and spectral misregistration when generating index rasters from 
    unstandardized photogrammetric outputs.
    """
...
class HydroBound_ML_App(QMainWindow):
    """
    Main application class for the HydroBound-ML tool.
    Handles UI generation, canvas rendering, spatial data processing, model training,
    and vector topology editing.
    """
    def __init__(self):
        super().__init__()
    ...
    def _setup_ui(self):
        """Constructs the application layout, assigning all widgets and layout groups."""
        central_widget = QWidget()
        self.setCentralWidget(central_widget)
        main_layout = QHBoxLayout(central_widget)

        # ---------------- LEFT PANEL (Main Pipeline) ----------------
        left_widget = QWidget()
        ...
        # --- STEP 2: PREDICTION & POST-PROCESSING ---
        ...
        self.spin_prob_thresh.setRange(0.10, 0.95); self.spin_prob_thresh.setValue(0.55); self.spin_prob_thresh.setSingleStep(0.05)
        self.spin_noise_filter = QDoubleSpinBox()
        self.spin_noise_filter.setRange(0.0, 100.0); self.spin_noise_filter.setValue(0.50); self.spin_noise_filter.setSingleStep(0.1)
        ...
        self.spin_obia_ratio.setRange(0.01, 0.99); self.spin_obia_ratio.setValue(0.20); self.spin_obia_ratio.setSingleStep(0.05)

        self.spin_smooth_radius = QDoubleSpinBox()
        self.spin_smooth_radius.setRange(0.0, 10.0); self.spin_smooth_radius.setSingleStep(0.5); self.spin_smooth_radius.setSuffix(" m")
        ...
        # --- STEP 3: MODEL TRAINING PARAMETERS ---
        ...
        self.spin_scale_2.setRange(0.2, 100.0); self.spin_scale_2.setValue(3.00); self.spin_scale_2.setSingleStep(1.0)
        self.spin_scale_3 = QDoubleSpinBox()
        self.spin_scale_3.setRange(0.3, 300.0); self.spin_scale_3.setValue(5.00); self.spin_scale_3.setSingleStep(1.0)
        ...
        self.spin_max_depth.setRange(0, 100); self.spin_max_depth.setValue(0); self.spin_max_depth.setSpecialValueText("Unlimited")
        self.spin_min_leaf = QSpinBox()
        self.spin_min_leaf.setRange(1, 100); self.spin_min_leaf.setValue(2)

        self.spin_jitter = QDoubleSpinBox()
        self.spin_jitter.setRange(0.0, 10.0); self.spin_jitter.setValue(1.00); self.spin_jitter.setSingleStep(0.5); self.spin_jitter.setSuffix(" m")
        self.spin_jitter_points = QSpinBox()
        self.spin_jitter_points.setRange(0, 50); self.spin_jitter_points.setValue(8)
        ...
        # =============================================================
        # 3. INTER-WIDGET CONNECTIONS & LOGIC
        # =============================================================
        ...
        # =============================================================
        # 4. APPEND GROUPS TO LAYOUT IN LOGICAL SEQUENCE
        # =============================================================
        layer_panel.addSpacing(10)
    ...
    def toggle_measure(self):
        """Toggles the interactive distance measurement tool."""
    ...
    def toggle_edit_mode(self):
        """Manages mutually exclusive states for vector editing tools."""
        sender = self.sender()
        ...
        self.toggle_measure() # visually reset measure logic
        self.clear_edit_drawing()

        if sender == self.btn_edit_del: self.edit_mode = 'del_poly'
        elif sender == self.btn_edit_fill: self.edit_mode = 'fill_hole'
        elif sender == self.btn_edit_add: self.edit_mode = 'add_poly'
        elif sender == self.btn_edit_cut: self.edit_mode = 'cut_poly'

    def clear_edit_drawing(self):
        """Clears transient vector drawing geometries from the matplotlib canvas."""
        self.edit_drawing_points = []
    ...
    def clear_measurements(self):
        """Clears transient measurement geometries from the matplotlib canvas."""
        self._measure_points = []
    ...
    def auto_update_postprocessing(self, micro_scale_val):
        """
        Dynamically couples post-processing parameters (LoD coupling).
        Ensures that vector smoothing and superpixel area are mathematically bound 
        to the model's minimum perception field (Scale 1).
        """
    ...
    def toggle_grid_settings(self):
        """Enables or disables DTM-based grid export UI components."""
        is_checked = self.check_export_grid.isChecked()
        self.spin_grid_size.setEnabled(is_checked)
        self.combo_z_method.setEnabled(is_checked)

    def find_latest_model_version(self):
        """Parses the MODEL directory to find the latest trained Random Forest iteration."""
        if not os.path.exists(self.model_dir): 
            return 0
        files = os.listdir(self.model_dir)
        versions = []
        for f in files:
            match = re.search(r"HydroBound-ML_v(\d+)\.joblib", f)
            if match: 
    ...
    def unlock_scales(self):
        """
        Unlocks scale parameters, permitting the user to reset the spatial context sizes.
        Requires deletion of the current model to maintain data consistency.
        """
    ...
    def update_sample_counter(self):
        """Updates the GUI label indicating current manual sampling quantities."""
    ...
    def clear_manual_samples_ui(self):
        """Clears transient training points from the matplotlib canvas."""
    ...
    def on_limits_changed(self, ax=None):
        """Triggers high-resolution fetching only when panning concludes."""
        if not self._pan_active: 
            self.render_timer.start(150) 

    def find_layer_path(self, layer_name):
        """Scans the INPUT directory to locate requested raster datasets."""
        candidates = ["DTM_ellipsoidal_heights.tif", "dtm_ellipsoidal_heights.tif", "DTM.tif", "dtm.tif", "DTM.TIF"] if layer_name == "dtm" else [f"{layer_name.upper()}.tif", f"{layer_name}.tif", f"{layer_name}.TIF"]
        for c in candidates:
    ...
    def get_layer_src(self, layer_name):
        """
        Manages file handlers for raster datasets. Generates visual contrast limits 
        using extreme percentiles derived securely from raster overviews (if available) 
        to prevent RAM saturation on massive ALB datasets.
        """
    ...
    def update_layer_ui(self):
        """Updates UI components reflecting current raster RAM/Disk cache status."""
    ...
    def check_and_generate_indices(self):
        """
        Validates the presence of required spectral indices (NDWI, NDVI, NGRDI).
        If missing, prompts the user to generate them. Leverages GDAL's WarpedVRT 
        for spatial realignment and executes via macro-chunking (4096px blocks) 
        to ensure memory efficiency and high-speed processing. Evaluates NoData 
        pixels accurately to prevent histogram corruption.
        """
        ...
        try:
            ...
            # --- HARD CLEANUP ---
            # Purge dict, invoke garbage collector and allow OS to fully flush disk buffers
            del dst_files
            gc.collect()
            time.sleep(1.0) 
    ...
    def load_project(self):
        """
        Loads the selected workspace. Triggers a robust hard-reset of memory caches 
        and spatial extents to guarantee strict isolation between projects.
        """
        ...
        self.selected_kml_path = ""
        if len(kml_files) > 1:
            item, ok = QInputDialog.getItem(self, "Select AOI File", "Multiple KML files found.\nPlease select the primary AOI file:", kml_files, 0, False)
            if ok and item:
    ...
    def fetch_high_res_raster(self):
        """
        Dynamically extracts and renders high-resolution raster subsets aligned with 
        the current canvas bounding box. Executes purely upon pan/zoom culmination.
        """
        if self._pan_active: 
            return
    ...
    def get_multiscale_features(self, mx, my, src_ndwi):
        """
        Extracts multi-scale (14D) spectral and spatial features for a specific point.
        Calculates pixel values, standard deviations across 3 window dimensions,
        and Gaussian Gradient Magnitudes.
        """
        feats = []
    ...
    def on_click(self, event):
        """
        Directs mouse interaction routing.
        Handles topology editing (Boolean Operations), distance measurement, 
        and manual ML point sampling simultaneously.
        """
        if event.button == 2: 
        ...
        # --- 3. ML MANUAL SAMPLING (CTRL+CLICK TO REMOVE) ---
    ...
    def train_model(self):
        """
        Executes Random Forest model training based on Spatial Otsu bootstrapping.
        Parses manually submitted training points, applies jitter augmentation, merges 
        with historical session data, and purges conflicting background (Otsu) instances.
        """
        v = self.current_model_version
        ...
        try:
            ...
            if len(X_b_tr) > 0:
                ...
                w_train = np.concatenate((np.ones(len(y_b_tr)), np.full(len(y_m_tr_expanded), self.spin_weight.value())))
            else:
            ...
            with open(log_path, "a") as f:
                ...
                if self.project_metadata:
                    f.write(f"Project Metadata -> Area: {self.project_metadata.get('project_info', {}).get('area_id', 'Unknown')}\n")
                f.write(f"Core-Set Filter: Water >= {w_min}, Land <= {l_max} (Otsu base reduced to {filtered_base_len} before limit)\n")
                f.write(f"Purged {deleted_points} Otsu points.\n")
                f.write(f"Manual Samples: {len(X_manual_all)} (Clones: x{repeats}, Noise: {self.spin_noise_aug.value()}, Weight: x{self.spin_weight.value()})\n")
                f.write(f"Model Gen: Max Depth: {max_depth_val}, Min Leaf: {min_leaf_val}\n")
                f.write(f"Validation Metrics (Global) -> Precision: {precision:.4f} | Recall: {recall:.4f} | F1: {f1:.4f} | IoU: {iou:.4f}\n")
    ...
    def predict_mask(self):
        """
        Executes semantic prediction (ML) and subsequent geometric decoupling via OBIA.
        Uses SLIC superpixels and morphological filtering to derive an accurate, sharp 
        vector mask corresponding to the predicted water extent.
        """
        src_ndwi = self.get_layer_src('ndwi')
        src_ndvi = self.get_layer_src('ndvi')
        src_ngrdi = self.get_layer_src('ngrdi')
        ...
        try:
            ...
            if self.selected_kml_path and os.path.exists(self.selected_kml_path):
                try:
                    ...
                    if not aoi_gdf.empty:
                        ...
                        aoi_window = aoi_window.intersection(rasterio.windows.Window(0, 0, src_ndwi.width, src_ndwi.height))
            ...
            if self.selected_kml_path and 'aoi_gdf' in locals() and not aoi_gdf.empty:
                ...
                kml_mask = rasterio.features.geometry_mask(aoi_geoms, out_shape=ndwi.shape, transform=aoi_transform, invert=True)
    ...
    def export_geojson(self):
        """
        Exports the vector boundaries to GeoJSON. Offers optional DTM 
        spatial intersection to attribute elevation data (z-method) across a regular grid.
        """
        if self.vector_mask is None or self.vector_mask.empty:
        ...
        try:
            ...
            if self.check_export_grid.isChecked():
                ...
                z_method = self.combo_z_method.currentText()
                ...
                p.setLabelText("Calculating Altitudes...")
                p.setValue(80)
                QCoreApplication.processEvents()
